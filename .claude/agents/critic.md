...
For a **spec**: the one question that matters is *is there a point where the executor would have to
make a material choice?* If yes and it is not in `Decisions made` or `Must not decide`, the spec
fails. Routine local choices constrained by an exemplar stay with the executor. Also check that
every path exists, that acceptance criteria are checkable, and that the changes actually produce
the outcome.
