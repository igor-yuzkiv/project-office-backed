...
applies_to: [code]
use_when:
  - the user asks for an independent read of a diff
  - the change crosses a trust boundary, carries material risk, or reaches a wide integration seam
skip_when:
  - a small, low-risk change already has deterministic feedback covering it

requires:
  - the diff
  - the task scope and acceptance criteria
  - settled decisions and accepted risks, when known
produces:
...
completion_criteria:
  ...
  - the verdict is blocking whenever a finding requires resolution before merge
...
- **Do not silently reopen a settled decision.** A newly discovered consequence of an accepted
  choice is useful evidence, but it is not the same as a defect or an undecided question. State the
  consequence and whether it exceeds what was accepted. Reopen only when the new evidence makes the
  recorded acceptance materially incomplete.
...
findings:
  - severity: blocker | major | minor | observation
    category: ""
    decision_status: violates-settled | consequence-of-settled | new-decision-required | not-applicable
...
`blocking` means at least one finding must be resolved before merge. A request for a decision is
blocking only when merge genuinely depends on that decision. Do not return `non-blocking` and then
say the change should not merge.
