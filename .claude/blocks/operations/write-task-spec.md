...
kind: operation
description: Produce a specification that carries material decisions while leaving routine exemplar-led judgement to the executor.
...
completion_criteria:
  ...
  - every material choice the executor could face is decided or listed as must-not-decide
...
**Is there any point at which the executor would have to make a material choice?**

If yes, that choice is either settled in `Decisions made` or listed in `Must not decide`.
Product behaviour, scope, shared contracts, security, data policy, and lasting trade-offs do not
become routine because the spec omitted them.

Local, reversible choices that follow an established exemplar remain with the executor. A spec that
tries to pre-decide those becomes longer, more brittle, and less useful.
...
- Do not leave a material decision open because its implementation looks small.
