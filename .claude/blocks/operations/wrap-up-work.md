...
If the task system distinguishes checkpoints from handoffs, use the distinction. A verified
intermediate state during an active implementation conversation is a checkpoint. Handoff only when
the user requests it or the implementation session is actually closing; later edits make an early
handoff stale.
