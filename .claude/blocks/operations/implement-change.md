...
requires:
  ...
  - established target and affected flow; a context map when orientation was needed
...
This operation makes routine implementation choices. If a material design decision is still open
when you reach it, that is an escalation, not a judgement call.
...
3. **Confirm the established context is still true.** When orientation produced a map, verify the
   paths and entry points you are about to touch. For direct work, verify the known target rather
   than creating a map as ceremony.
...
- Do not silently settle a material choice the spec left open — escalate it instead.
