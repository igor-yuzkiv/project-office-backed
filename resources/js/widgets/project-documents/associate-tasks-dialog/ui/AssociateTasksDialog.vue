...
// TODO: Fetches the authoritative full list of already-associated tasks (not the page's possibly-paginated view) since Save replaces the entire association in one sync call.
...
// Guards against wiping in-progress user edits: the authoritative currentTasks fetch
...
let hasSyncedCurrentTasks = false
...
watch(visible, (isVisible) => {
    if (isVisible) {
        hasSyncedCurrentTasks = false
        initDraft()
    }
})

watch(currentTasks, () => {
    if (visible.value && !hasSyncedCurrentTasks) {
        initDraft()
        hasSyncedCurrentTasks = true
    }
})
</script>
