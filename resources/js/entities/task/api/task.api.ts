...
export async function fetchTasksRequest(projectId: string, params?: PagingParams & SortParams): PromisePaginatedResponse<ITask> {
    return httpClient
        .get<PaginatedResponse<ITask>>(`/projects/${projectId}/tasks`, { params })
        .then((res) => res.data)
}
...
export async function updateTaskRequest(projectId: string, taskId: string, data: IUpdateTaskInput): Promise<TaskResponse> {
...
export async function deleteTaskRequest(projectId: string, taskId: string): Promise<{ message: string }> {
    return httpClient
        .delete<{ message: string }>(`/projects/${projectId}/tasks/${taskId}`)
        .then((res) => res.data)
}
