...
export async function fetchTaskListsRequest(projectId: string, params?: PagingParams & SortParams): PromisePaginatedResponse<ITaskList> {
    return httpClient
...
export async function fetchTaskListRequest(projectId: string, taskListId: string): Promise<TaskListResponse> {
    return httpClient
        .get<TaskListResponse>(`/projects/${projectId}/task-lists/${taskListId}`)
        .then((res) => res.data)
}
...
export async function updateTaskListRequest(projectId: string, taskListId: string, data: IUpdateTaskListInput): Promise<TaskListResponse> {
    return httpClient
