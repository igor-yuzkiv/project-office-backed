...
<template>
    ...
    >
        <div class="h-14 flex shrink-0 items-center" :class="layoutStore.sidebarCollapsed ? 'justify-center px-0' : 'gap-2.5 px-4'">
        ...
        <nav class="gap-1 px-2 flex flex-col">
            <SidebarNavLink v-for="item in items" :key="item.key" :item="item" :collapsed="layoutStore.sidebarCollapsed" />
        </nav>
        ...
        <div
            class="gap-0.5 border-surface-700 px-2 py-2 mt-auto flex flex-col border-t"
        >
            <button
                class="rounded-md text-sm text-surface-300 hover:bg-surface-800 hover:text-surface-0 flex items-center transition-colors"
                :class="layoutStore.sidebarCollapsed ? 'justify-center p-2' : 'gap-3 px-3 py-2'"
